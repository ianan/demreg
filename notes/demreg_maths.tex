...
The horizontal error on the DEM solution are the ``temperature resolution'' and found from measuring the spread of non-diagonal terms in ${\bf K R_\lambda}$, since the better the solution $\boldsymbol{\xi}_\lambda = {\bf R}_\lambda {\bf g}$ then the more ${\bf K R_\lambda} \approx {\bf I}$.
