FUNCTION data2dem_reg_temps, tresp_logT ,TRmatrix ,data ,edata ,temps,$
  ...
  ; INPUTS:
  ;
  ; 	tresp_logT  		- Temperatures where the filter responses/contribution functions are known [log10(K)]
  ...
  ; 28-Oct-2025 Changed to use/specify temperature bin edges,
  ;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
;    for ii=0, nf-1 do emloci[*,ii]=data[ii]/(TRmatint[*,ii]*10d^mlogT*alog(10d^dlogT))
    ...
    L=dblarr(nT,nT)
    for i=0, nT-1 do L[i,i]=1.0/sqrt(dlogT[i])
